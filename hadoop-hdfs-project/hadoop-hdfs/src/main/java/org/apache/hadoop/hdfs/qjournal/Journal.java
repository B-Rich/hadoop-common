...
import org.apache.hadoop.hdfs.util.PersistentLong;
...
class Journal implements Closeable {
  ...
  public synchronized PrepareRecoveryResponseProto.Builder prepareRecovery(
      RequestInfo reqInfo, long segmentTxId) throws IOException {
    checkRequest(reqInfo);
    
    PrepareRecoveryResponseProto.Builder resp = PrepareRecoveryResponseProto.newBuilder();
    ...
    LOG.info("Prepared recovery for segment " + segmentTxId + ": " +
        TextFormat.shortDebugString(resp.clone().buildPartial()));
    return resp;
  }
