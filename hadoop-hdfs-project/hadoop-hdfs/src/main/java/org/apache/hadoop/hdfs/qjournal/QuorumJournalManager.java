...
public class QuorumJournalManager implements JournalManager {
  ...
  private static final int PREPARE_RECOVERY_TIMEOUT_MS = 20000;
  private static final int ACCEPT_RECOVERY_TIMEOUT_MS = 20000;
  private static final int FINALIZE_SEGMENT_TIMEOUT_MS = 20000;
  ...
  private void recoverUnclosedSegment(long segmentTxId) throws IOException {
    ...
    Map<AsyncLogger, PrepareRecoveryResponseProto> prepareResponses=
        loggers.waitForWriteQuorum(prepare, PREPARE_RECOVERY_TIMEOUT_MS);
    ...
    // test for it: write a stress test which just runs through recovery
    // in a tight loop.
    
    URL syncFromUrl = bestLogger.buildURLToFetchLogs(segmentTxId);
    
    QuorumCall<AsyncLogger,Void> accept = loggers.acceptRecovery(logToSync, syncFromUrl);
    loggers.waitForWriteQuorum(accept, ACCEPT_RECOVERY_TIMEOUT_MS);
    ...
    loggers.waitForWriteQuorum(finalize, FINALIZE_SEGMENT_TIMEOUT_MS);
  }
